...
export default class Ticket {
	...
	async save () {
		const connection = pgp()("postgres://postgres:123456@localhost:5432/app");
		await connection.query("insert into ticket (ticket_id, requester_id, content, start_date, status) values ($1, $2, $3, $4, $5)", [this.ticketId, this.requesterId, this.content, this.startDate, this.status]);
		await connection.$pool.end();
	}

	async update () {
		const connection = pgp()("postgres://postgres:123456@localhost:5432/app");
		await connection.query("update ticket set status = $1, assignee_id = $2, end_date = $3, duration = $4 where ticket_id = $5", [this.status, this.assigneeId, this.endDate, this.duration, this.ticketId]);
		await connection.$pool.end();
	}
